...
export default function GestionPedidosPage() {
  ...
  if (estado === 'cargando') return <p className="text-gray-800">Cargando...</p>
  ...
  return (
      <div className="min-h-screen bg-gradient-to-br from-blue-100 to-purple-200 py-8 px-2 flex flex-col items-center">
        <div className="w-full max-w-5xl bg-white rounded-xl shadow-xl p-6 animate-fade-in text-gray-800">
          <div className="flex justify-between items-center mb-6">
            <Link href="/admin" className="text-gray-700 hover:underline text-sm">← Volver al panel</Link>
          ...
          <form className="flex flex-wrap gap-4 mb-4 items-end">
            ...
            <div>
              ...
              <input type="date" value={fechaInicio} onChange={e => setFechaInicio(e.target.value)} className="border rounded px-2 py-1 text-gray-800" />
            </div>
            <div>
              <label className="block text-xs font-semibold mb-1 text-gray-800">Fecha fin</label>
              <input type="date" value={fechaFin} onChange={e => setFechaFin(e.target.value)} className="border rounded px-2 py-1 text-gray-800" />
            </div>
          ...
          {loading ? (
              <div className="text-center py-8 text-gray-800">Cargando...</div>
          ) : (
                ...
                <table className="min-w-full border text-sm text-gray-800">
                  ...
                  {pedidos.map(p => (
                      <tr key={p.id_pedido} className="border-b hover:bg-purple-50">
                        ...
                        <td className="p-2 text-center">
                          <Link href={`/admin/pedidos/${p.id_pedido}`} className="text-blue-700 hover:underline">Ver detalles</Link>
                        </td>
                      </tr>
                  ))}
                  </tbody>
          ...
          <div className="flex justify-center gap-2 mt-4">
            ...
            <span className="px-2 text-gray-800">Página {page} de {totalPages}</span>
